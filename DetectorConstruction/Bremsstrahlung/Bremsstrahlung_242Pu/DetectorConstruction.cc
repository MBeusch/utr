...
G4VPhysicalVolume *DetectorConstruction::Construct() {
	...
	bremstarget_thickness1 = 2.5*mm;
	distance_bremstargets = 15*mm;  //guesstimated distance between bremstargets
	const double bremstarget_edge_length = 10*mm;
	const double beamhardener_thickness = 5*mm;
	const double beamhardener_edge_length = 100*mm;
	const double steelpipe_radius = 25*mm;
	const double steelpipe_thickness = 2.5*mm;
	...
	/******************** Beam hardener ******************/
	G4Box *Beamhardener_solid = new G4Box("Beamhardener", beamhardener_edge_length * 0.5, beamhardener_edge_length * 0.5, beamhardener_thickness * 0.5);
	G4LogicalVolume *Beamhardener_logical = new G4LogicalVolume(Beamhardener_solid, copper, "Beamhardener_logical", 0, 0, 0);

	//Visualisierung (Farbe)
	Beamhardener_logical->SetVisAttributes(new G4VisAttributes(G4Color::Brown()));
	...
	//new G4PVPlacement(nullptr, G4ThreeVector(0, 0, targetposition_z - distance_target_container - 0.5 * container_thickness), container_logical, "ContainerBottom", World_logical, false, 0);


	/******************** Steelpipe flange ******************/ 
	G4Tubs *steelpipe_solid = new G4Tubs("Steelpipe_solid", 0, steelpipe_radius, 0.5 * steelpipe_thickness, 0, twopi);
	G4LogicalVolume *steelpipe_logical = new G4LogicalVolume(steelpipe_solid, peek, "Steelpipe_logical");
	steelpipe_logical->SetVisAttributes(G4Color::Grey());
	new G4PVPlacement(nullptr, G4ThreeVector(0, 0, targetposition_z - distance_container_steelpipe - container_thickness - 0.5 * steelpipe_thickness), steelpipe_logical, "SteelpipeBottom", World_logical, false, 0);
